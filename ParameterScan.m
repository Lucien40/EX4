...
ua        = 149597870700; %Astronomical unit
%Earth:
mT = 5.972e24; %kg
rT = 6378.1e3; %m
 ...
 % Parametres physiques :
    ...
    switch Ex
        case '2a'
            ...
            grid on
            hold on
            pHal = plot(ax,x1_2/ua,x2_2/ua,'.');
            viscircles(ax,[x1_2(1)/ua x2_2(1)/ua],rL/ua,'Color',colors(2,:));
            pT = plot(ax,x1_1/ua,x2_1/ua,'.');
            viscircles(ax,[x1_1(1)/ua x2_1(1)/ua],rT/ua,'Color',colors(4,:));
    ...
    ' %s=%s'] , ...
        ...
        name = [Ex,'.out'];
            ...
            if (Resimulate) %test if the file exists
                ...
                system(cmd); % strcat("wsl ",cmd) Wsl to compile using gcc on the wsl (windows subsystem for linux)
            end
